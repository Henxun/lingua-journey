...
export default function Login() {
  ...
  const handleOAuthLogin = async (provider: 'google' | 'github') => {
    try {
      ...
      if (!data.auth_url) {
        throw new Error('Failed to get authentication URL. Please check if OAuth credentials are configured.');
      }
  ...
  const handleVerifyCode = async (e: React.FormEvent) => {
    ...
    try {
      setLoading(true);
      const data = await authAPI.verifyEmailCode({ email, code, type: 'login' });
    ...
    <div className="min-h-screen bg-gradient-to-br from-blue-50 via-white to-indigo-50 flex items-center justify-center px-4 py-12">
      <div className="max-w-md w-full bg-white rounded-2xl shadow-xl overflow-hidden">
        <div className="bg-gradient-to-r from-blue-600 to-indigo-600 px-8 py-10 text-center">
          ...
          <p className="text-blue-100">{t('auth.login.subtitle')}</p>
        </div>
            ...
            <form onSubmit={handleVerifyCode} className="space-y-4">
              ...
              {!showCodeInput ? (
                <button
                  ...
                  className="w-full bg-gradient-to-r from-blue-600 to-indigo-600 text-white py-3 px-4 rounded-xl font-medium hover:from-blue-700 hover:to-indigo-700 transition-colors disabled:opacity-50"
                >
                  {countdown > 0 ? `${t('auth.login.resendIn')} ${countdown}s` : loading ? t('auth.login.sending') : t('auth.login.sendCode')}
                </button>
              ) : (
                <>
                  <div>
                    <label className="block text-sm font-medium text-gray-700 mb-2">{t('auth.login.verificationCode')}</label>
                    <input
                      type="text"
                      value={code}
